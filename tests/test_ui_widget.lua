...
t.test("field.format never exceeds width when label fills the field", function()
  t.eq(#widget.field.format("HDG", "359.9", 4), 4, "exact width on overflow")
  t.eq(widget.field.format("HDG", "359.9", 4), "HDG ", "value dropped, one gap")
  t.eq(#widget.field.format("TOOLONG", "x", 4), 4, "label longer than width truncates to width")
end)
