...
-- headingSignScale: set signHeading CONSISTENT with the yawRate sensor observed during the SAME
-- rotation, so the heading loop is negative feedback (Flight #9 spun because the two disagreed).
t.test("headingSignScale: heading and yawRate agree -> sign +1", function()
  local r = cal.headingSignScale(0, 1.5, 0.3)
  t.eq(r.sign, 1); t.eq(r.unit, "rad"); t.near(r.scale, 1, 1e-9); t.eq(r.status, "ok")
end)
t.test("headingSignScale: heading reads BACKWARDS vs yawRate -> sign -1 (the Flight #9 bug)", function()
  -- raw heading DECREASED while the craft yawed positive (yawRate +) -> flip signHeading
  local r = cal.headingSignScale(0, -1.5, 0.3)
  t.eq(r.sign, -1); t.eq(r.status, "ok")
end)
t.test("headingSignScale: negative-vs-negative also agrees -> sign +1", function()
  t.eq(cal.headingSignScale(0, -1.5, -0.3).sign, 1)
end)
t.test("headingSignScale: 90deg rotation still detected as degrees", function()
  local r = cal.headingSignScale(0, 90, 0.3)
  t.eq(r.unit, "deg"); t.near(r.scale, math.pi/180, 1e-9)
end)
t.test("headingSignScale: yawRate too small to trust -> too-small (rotate faster)", function()
  local r = cal.headingSignScale(0, 1.5, 0.001)
  t.eq(r.status, "too-small"); t.eq(r.yawOk, false)
end)
t.test("headingSignScale: heading change too small -> too-small", function()
  t.eq(cal.headingSignScale(0, 0.001, 0.3).status, "too-small")
end)
