...
t.test("cachedForMode returns the same table until the mode id changes", function()
  local get = keymap.cachedForMode()
  local a = get("PRECISION")
  local b = get("PRECISION")
  t.eq(a, b, "same mode => same table identity (not a fresh forMode call)")
  t.eq(a, keymap.default)
  local c = get("CPL")
  t.eq(c, keymap.coupled, "CPL switches to the coupled map")
  t.eq(get("DCPL"), keymap.coupled)
  local d = get("PRECISION")
  t.eq(d, keymap.default, "switching back refreshes")
  t.eq(get("PRECISION"), d, "stable again")
end)
