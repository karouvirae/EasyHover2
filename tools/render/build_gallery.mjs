...
const GROUPS = [
  ...
  { name: "NAV & BIT/CONFIG", sub: "2w×1h monitors · 36×10 · NAV page + its drilldowns", panels: [
    ["nav", "NAV"], ["hub", "BIT/CONFIG Hub"], ["tuning", "FCS TUNING"], ["tuning_edit", "FCS TUNING ▸ edit gains"],
    ["mdb", "MDB-CONF"], ["uical", "UI CAL"], ["uical_settings", "UI CAL ▸ UI SETTINGS"], ["senscal", "SENS CAL"],
...
const html = `<title>EasyHover 2 Cockpit</title>
...
<div class="wrap">
  <p class="eyebrow">EasyHover 2 · Cockpit · Current state</p>
  <h1>Every cockpit panel, as it flies today</h1>
  <p class="lede">A faithful capture of the <b>entire</b> EasyHover 2 cockpit — every page, region
    drilldown, BIT/CONFIG submenu and entry overlay — grouped by the physical monitor surface it runs
    on. These are not mockups: each image is the real Basalt UI, mounted headless in CraftOS-PC and
    captured cell-for-cell exactly as it <code>blit</code>s in-game.</p>
  <p class="lede">The pilot flies the <b>PFD</b> (heading tape · attitude · ALT/SPD · waypoint cue),
    monitors engine + flight control on the <b>Overhead FLIGHT</b> stack, and runs navigation, waypoints
    and all setup from the <b>NAV</b> surface and its BIT/CONFIG drilldowns. <b>CONFIG</b> lives on the
    UI-PC terminal; the <b>A/P</b> panel is its own small monitor. Panels render at
    <b><code>default / unbound</code></b> state, so live values show as placeholders — layout, chrome and
    colour are exact.</p>
